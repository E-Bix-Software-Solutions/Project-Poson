...
const CardGenerator: React.FC<CardGeneratorProps> = ({ onBackToLanding }) => {
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-800 font-sans antialiased selection:bg-amber-100 selection:text-amber-900">
      ...
      <nav className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center border-b border-slate-200/80">
        <button
          onClick={onBackToLanding}
          className="flex items-center space-x-2 text-sm text-slate-600 hover:text-amber-600 transition-colors group"
        >
        ...
        <div className="flex flex-col items-end">
          <span className="text-xs font-mono tracking-widest text-cyan-600 font-bold">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <section className="lg:col-span-5 space-y-6 bg-white border border-slate-200/80 p-6 rounded-2xl h-fit shadow-sm">
          <div>
          ...
          <div className="space-y-2">
            <label className="block text-xs font-mono uppercase text-slate-500 tracking-wider">
            ...
            <div className="grid grid-cols-2 gap-2">
              {presetMessages.map((msg, idx) => (
                <button
                  ...
                  className="px-3 py-2 bg-slate-50 border border-slate-200 text-left text-xs rounded-lg hover:border-slate-350 hover:bg-slate-100/80 transition-all truncate text-slate-600"
                >
                  {msg.label}
                </button>
              ))}
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              rows={4}
              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm text-slate-800 focus:outline-none focus:border-amber-500 focus:bg-white transition-all resize-none font-sans leading-relaxed"
          ...
          <div className="space-y-4 pt-2 border-t border-slate-100">
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-mono text-slate-500">
                ...
                <span className="text-amber-600 font-bold">{fontSize}px</span>
              </div>
              <input
                type="range"
                min="16"
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs font-mono text-slate-500">
                ...
                <span className="text-amber-600 font-bold">{paddingSize}px</span>
              </div>
              <input
                type="range"
                min="20"
        ...
        <section className="lg:col-span-7 flex flex-col justify-between space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-mono uppercase tracking-widest text-slate-500">
            ...
            <div
              ...
              className={`w-full aspect-[16/10] sm:aspect-[16/9] ${selectedTheme.className} rounded-2xl border border-slate-200/50 relative flex flex-col justify-between overflow-hidden shadow-lg transition-all duration-300 group`}
            >
              {/* BACKDROP HIGH INTENSITY FULL MOON VECTOR SOURCE */}
              <div className="absolute top-8 right-12 w-28 h-28 sm:w-36 sm:h-36 bg-gradient-to-br from-yellow-100 via-white to-amber-200 rounded-full opacity-80 blur-[2px] shadow-[0_0_50px_rgba(254,243,199,0.3)] flex items-center justify-center -z-0">
                {/* Internal Moon surface shadow detail mock */}
                <div className="w-full h-full bg-black/5 rounded-full filter contrast-125 mix-blend-multiply opacity-20"></div>
              </div>

              {/* WATER ripple/lantern ambient background highlights */}
              <div className="absolute bottom-0 inset-x-0 h-1/3 bg-gradient-to-t from-slate-900/5 via-transparent to-transparent pointer-events-none"></div>

              {/* CARD PREVIEW TOP META BAND */}
              <div className="w-full flex justify-between items-start z-10 relative">
                <div>
                  {showLotus && (
                    <div className="text-2xl drop-shadow-[0_0_8px_rgba(8,145,178,0.3)] animate-pulse">
                      🪷
                    </div>
                  )}
                </div>
                <div className="text-right font-mono text-[9px] uppercase tracking-widest text-slate-800/40">
              ...
              <div className="w-full max-w-xl my-auto z-10 relative py-4">
                <p
                  ...
                  className={`font-sans font-bold leading-relaxed tracking-wide drop-shadow-[0_2px_4px_rgba(0,0,0,0.05)] text-balance ${selectedTheme.textColor}`}
                >
                  {customText || "..."}
                </p>
              </div>
              ...
              <div className="w-full flex justify-between items-end pt-4 border-t border-slate-900/10 z-10 relative">
                {/* Left Side Group: Traditional Silhouettes Simulation */}
                <div className="flex items-end space-x-4 min-h-[36px]">
                  {showStupa && (
                    <div className="flex flex-col items-center select-none text-slate-800/80 filter drop-shadow-sm">
                      <span className="text-2xl leading-none">🛕</span>
                      <span className="text-[8px] font-mono tracking-tighter uppercase opacity-55 mt-0.5">
                        Mihintale
                      </span>
                    </div>
                  )}
                  {showDeer && (
                    <div className="flex flex-col items-center select-none text-amber-700 filter drop-shadow-sm">
                      <span className="text-2xl leading-none">🦌</span>
                      <span className="text-[8px] font-mono tracking-tighter uppercase opacity-55 mt-0.5">
                        Wisdom
                      </span>
                ...
                {showBranding ? (
                  <div className="text-right flex flex-col items-end opacity-90">
                    ...
                    <span className="text-[8px] font-mono text-slate-800/40 uppercase tracking-tighter">
                      Software Ecosystem Pipeline
                    </span>
                  </div>
                ) : (
                  <div className="w-4 h-4 rounded bg-slate-900/5"></div>
                )}
              </div>

              {/* Ambient Buddhist Flag Aspect Gradient Border Bar Accent */}
              <div className="absolute top-0 inset-x-0 h-1 grid grid-cols-5 opacity-60">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-sm">
            <div className="text-xs text-slate-500 text-center sm:text-left">
              <p className="font-mono text-[11px] text-slate-700 uppercase font-semibold">
            ...
            <button
              ...
              className={`w-full sm:w-auto px-6 py-3 font-bold rounded-xl text-xs tracking-wide transition-all uppercase flex items-center justify-center space-x-2 ${
                isExporting
                  ...
                  : "bg-gradient-to-r from-amber-500 via-yellow-400 to-amber-600 text-slate-950 hover:scale-[1.01] font-extrabold shadow-md shadow-amber-500/10"
              }`}
            >
              {isExporting ? (
                <>
          ...
          {exportSuccess && (
            <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-start space-x-3 text-xs text-emerald-800 animate-fadeIn">
      ...
      <footer className="bg-white border-t border-slate-200/80 py-8 text-center text-slate-500 text-xs mt-16">
        <p>
